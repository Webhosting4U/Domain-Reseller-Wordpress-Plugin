...
class WH4U_Admin_Appearance {
	...
	private static function render_section_typography( $s ) {
		...
		<div class="wh4u-appearance-section">
			...
			<table class="form-table">
				...
				<tr id="wh4u-custom-font-row" style="<?php echo esc_attr( $s['font_family'] !== 'custom' ? 'display:none;' : '' ); ?>">
	 ...
	 *
	 * Uses wp_enqueue_style / wp_enqueue_script with wp_head() / wp_footer() so that
	 * all CSS and JS are loaded through the WordPress enqueue system.
	 *
	 * @return void
	 */
	public static function render_preview() {
		...
		// Enqueue both stylesheets; the inactive one will be disabled via style_loader_tag filter.
		wp_enqueue_style(
			'wh4u-preview-full-css',
			WH4U_DOMAINS_PLUGIN_URL . 'public/css/wh4u-public.css',
			array(),
			WH4U_DOMAINS_VERSION
		);
		wp_enqueue_style(
			'wh4u-preview-minimal-css',
		...
		wp_add_inline_style( 'wh4u-preview-full-css', $preview_inline_css );
		wp_add_inline_style( 'wh4u-preview-minimal-css', $preview_inline_css );

		// Add disabled attribute to the inactive stylesheet so both are in the DOM for postMessage toggling.
		add_filter( 'style_loader_tag', function ( $tag, $handle ) use ( $css_mode ) {
			if ( 'wh4u-preview-full-css' === $handle && 'full' !== $css_mode ) {
				$tag = str_replace( " id='wh4u-preview-full-css-css'", " id='wh4u-preview-full-css-css' disabled", $tag );
			}
			if ( 'wh4u-preview-minimal-css' === $handle && 'minimal' !== $css_mode ) {
				$tag = str_replace( " id='wh4u-preview-minimal-css-css'", " id='wh4u-preview-minimal-css-css' disabled", $tag );
			}
			return $tag;
		}, 10, 2 );
		...
		wp_enqueue_script(
			'wh4u-preview-js',
			WH4U_DOMAINS_PLUGIN_URL . 'admin/js/wh4u-preview.js',
			array(),
			WH4U_DOMAINS_VERSION,
			true
		);
		wp_localize_script( 'wh4u-preview-js', 'wh4uPreview', array(
